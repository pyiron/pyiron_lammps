...
class TestCompatibilityFile(unittest.TestCase):
    def setUp(self):
        ...
        self.output_key_lst = [
            'steps', 'natoms', 'cells', 'indices', 'forces', 'velocities', 'positions',
            'temperature', 'energy_pot', 'energy_tot', 'volume', 'pressures',
        ]
        self.output_key_1d_lst = ['steps', 'natoms', 'temperature', 'energy_pot', 'energy_tot', 'volume']
    ...
    def test_calc_md_langevin_nvt(self):
        calc_kwargs = {"temperature": 500.0, "n_ionic_steps": 1000, "n_print": 100, "langevin": True}
        shell_output, parsed_output, job_crashed = lammps_file_interface_function(
            working_directory=self.working_directory,
            structure=self.structure,
            potential=self.potential,
    ...
    def test_calc_md_langevin_npt(self):
        calc_kwargs = {"temperature": 500.0, "pressure": 0.0, "n_ionic_steps": 1000, "n_print": 100, "langevin": True}
        shell_output, parsed_output, job_crashed = lammps_file_interface_function(
            working_directory=self.working_directory,
            structure=self.structure,
            potential=self.potential,
